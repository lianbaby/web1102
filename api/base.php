...
class DB{
    protected $dsn="mysql:host=localhost;charset=utf8;dbname=db1102";
    protected $table;
    protected $pdo;
    public $type=[
                1=>'健康新知',
...
function q($sql){
    $pdo=new PDO("mysql:host=localhost;charset=utf8;dbname=db1102",'root','');
...
if(!isset($_SESSION['total'])){
    ...
    if(empty($today)){ //沒有今天的資料，新增
        $today=['date'=>date("Y-m-d"),'total'=>1];
    }else{
        $today['total']++;// 有今天的資料，更新
    }
    $Total->save($today);   //存檔 
    $_SESSION['total']=1;  //回傳
}
